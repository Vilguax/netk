...
type OAuthProviderId = "google" | "discord";
...
function LoginPageContent() {
  ...
  const [availableProviders, setAvailableProviders] = useState<OAuthProviderId[]>([]);
    ...
    void (async () => {
      try {
        ...
        setAvailableProviders(
          (["google", "discord"] as const).filter((provider) => Boolean(providers[provider]))
        );
      } catch {
        if (!cancelled) {
          setAvailableProviders([]);
    ...
    try {
      ...
      if (result?.error) {
        if (result.error === "EMAIL_NOT_VERIFIED") {
          setFormError("Veuillez vérifier votre email avant de vous connecter.");
        } else {
          ...
          <div className="relative p-8">
            ...
            {(error || formError) && (
              <div className="mb-6 p-4 rounded-lg bg-red-500/10 border border-red-500/30 text-red-400 text-sm">
                {formError || "Une erreur est survenue lors de la connexion."}
              </div>
            )}
                ...
                <div className="space-y-3">
                  {availableProviders.includes("google") && (
                    <button
                      ...
                      className="w-full py-3 px-4 rounded-lg bg-slate-800/50 border border-slate-700/50 text-white font-medium hover:bg-slate-800 hover:border-orange-500/30 focus:outline-none focus:ring-2 focus:ring-orange-500/30 transition-all duration-300 flex items-center justify-center gap-3"
                    >
                  ...
                  {availableProviders.includes("discord") && (
                    <button
                      ...
                      className="w-full py-3 px-4 rounded-lg bg-slate-800/50 border border-slate-700/50 text-white font-medium hover:bg-slate-800 hover:border-[#5865f2]/30 focus:outline-none focus:ring-2 focus:ring-[#5865f2]/30 transition-all duration-300 flex items-center justify-center gap-3"
                    >
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-black text-white p-6">Chargement...</div>}>
      <LoginPageContent />
    </Suspense>
  );
}
